...
from lib.utils.bbox_transform import bbox_transform_inv, bbox_transform_inv_r
...
def im_detect(sess, net, im):
    ...
    _, scores, ro_bbox_pred, rois = net.test_image(sess, blobs['data'], blobs['im_info'])

    boxes = rois[:, 1:5] / im_scales[0]
    scores = np.reshape(scores, [scores.shape[0], -1])
    ro_bbox_pred = np.reshape(ro_bbox_pred, [ro_bbox_pred.shape[0], -1])

    # Apply bounding-box regression deltas
    box_deltas_r = ro_bbox_pred

    pred_boxes_r = bbox_transform_inv_r(boxes, box_deltas_r)

    # pred_boxes_r = _clip_boxes(pred_boxes_r, im.shape)


    return scores, pred_boxes_r
